class Dividend {
  ...
  factory Dividend.fromJson(Map<String, dynamic> json) => Dividend(
    ...
    notes: json['notes'],
    assetName:
        json['asset_name'] ?? json['assetName'] ?? json['asset']?['name'],
    ticker: json['ticker'] ?? json['asset']?['ticker'],
    logoUrl:
        json['logo_url'] ??
        json['logoUrl'] ??
        json['asset']?['logo_url'] ??
...
class MonthlyDividend {
  ...
  factory MonthlyDividend.fromJson(Map<String, dynamic> json) =>
      MonthlyDividend(
        month:
            json['label'] ??
            '${json['year']}-${json['month'].toString().padLeft(2, '0')}',
        totalUsd: ((json['total_usd'] ?? json['total_amount'] ?? 0) as num)
            .toDouble(),
